...
Owner/admin может проверить локальную конфигурацию отправителя через
`GET /api/v1/email/deliverability`. Проверка сверяет адрес `From` с SMTP-аккаунтом,
но не заявляет, что SPF, DKIM или DMARC успешно настроены: DNS-записи нужно
проверить извне, а реальную доставку — тестовым письмом через mail-tester.com.

## Обновления inbox в реальном времени

Frontend поддерживает одно авторизованное SSE-соединение с
`GET /api/v1/conversations/events`. Bearer-токен передаётся в заголовке
`Authorization`, а не в URL; поток ограничен tenant текущего пользователя.
Клиент переподключается с увеличивающейся задержкой, а пока поток недоступен,
использует резервный REST polling.
